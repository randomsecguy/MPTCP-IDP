...
The above figure explains the assumed setup. All the traffic from the MPTCP client and the TCP server needs to traverse through proxy. When the proxy starts, it sets up NAT rules to be able to transparently communicate with client on the behalf of TCP server and with the server on behalf of the client. 
